...
app.initClickEvents = function () {
    function handleClicks(e) {
        ...
        var clicked = $(this),
            ...
            isLink = clicked[0].nodeName.toLowerCase() === 'a',
            panel, popover, popup,
            text, title,
            validUrl, view, i, animatePages;
        // External
        ...
        if (clicked.hasClass('open-panel')) {
            ...
            if (panel.length === 1) {
                ...
                if (clicked.attr('data-panel') === 'right') {
                    app.openPanel('right');
                }
                else {
                    app.openPanel('left');
        ...
        if (clicked.hasClass('swipeout-delete')) {
            if (clicked.attr('data-confirm')) {
                text = clicked.attr('data-confirm');
                title = clicked.attr('data-confirm-title');
                if (title) {
        ...
        validUrl = url && url.length > 0 && url.indexOf('#') !== 0;
        if (validUrl || clicked.hasClass('back')) {
            ...
            if (clicked.hasClass('back')) {
                view.goBack(clicked.attr('href'), animatePages);
            }
            else {
                view.loadPage(clicked.attr('href'), animatePages);
            }
        }
    }
