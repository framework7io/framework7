...
app.addNotification = function (params) {
    var container, list, itemHTML, item, itemHeight, clientLeft;
    if (!params) {
        return;
    }
    ...
    item = $(_tempNotificationElement).children();
    ...
    item.on('click', function (e) {
        ...
        else if ($(e.target).is('.close-notification') || $(e.target).parents('.close-notification').length > 0) {
            app.closeNotification(item[0]);
        }
    });
    if (params.onClose) {
...
app.closeNotification = function (item) {
    var container, itemHeight, clientLeft;
    item = $(item);
    if (item.length === 0) {
        return;
    }
