...
app.initSmartSelects = function (pageContainer) {
    var page = $(pageContainer), selects;
    if (page.length === 0) {
        return;
    }
    ...
    selects.each(function () {
        var smartSelect = $(this),
            $select = smartSelect.find('select'),
            select, valueText, i, itemAfter;
        if ($select.length === 0) {
            return;
        }
        ...
        for (i = 0; i < select.length; i++) {
            if (select[i].selected) {
                valueText = select[i].textContent.trim();
            }
        }

        itemAfter = smartSelect.find('.item-after');
...
app.smartSelectOpen = function (smartSelect) {
    var view, select, values, i, pageTitle, backText, id, radiosName, radiosHTML, j, checked, navbarHTML,
        navbarLayout, pageName, noToolbar, noNavbar, useSearchbar, searchbarPlaceholder, searchbarCancel,
        searchbarHTML, pageHTML;
    ...
    // Find related view
    view = smartSelect.parents('.' + app.params.viewClass);
    if (view.length === 0) {
        return;
    }
    view = view[0].f7View;
    ...
    // Collect all values
    select = smartSelect.find('select')[0];
    values = {};
    ...
    pageTitle = smartSelect.attr('data-pagetitle') || smartSelect.find('.item-title').text();
    backText = smartSelect.attr('data-backtext') || app.params.smartSelectBackText;
    ...
    for (j = 0; j < values.length; j++) {
        checked = values[j].selected ? 'checked' : '';
        radiosHTML +=
            '<li>' +
    ...
    // Page Layout
    pageName = 'smart-select-' + radiosName;

    noToolbar = smartSelect.parents('.page').hasClass('no-toolbar') ? 'no-toolbar' : '';
    noNavbar = smartSelect.parents('.page').hasClass('no-navbar') ? 'no-navbar' : '';

    useSearchbar = typeof smartSelect.data('searchbar') === 'undefined' ? app.params.smartSelectSearchbar : (smartSelect.data('searchbar') === 'true' ? true : false);
    ...
    searchbarHTML = '<form class="searchbar" data-search-list=".smart-select-list-' + id + '" data-search-in=".item-title">' +
    ...
    function handleRadios(e) {
        ...
        if (page.name === pageName) {
            ...
            $(page.container).find('input[name="' + radiosName + '"]').on('change', function () {
                var value = this.value, optionText;
                select.value = value;
