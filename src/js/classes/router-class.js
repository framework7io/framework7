...
import { forward as RouterForward, load as RouterLoad } from './router/load';
import { backward as RouterBackward, back as RouterBack } from './router/back';
...
class Router {
  ...
  init() {
    const router = this;
    if (router.$pagesEl.find('.page:not(.stacked)').length === 0 && router.params.url) {
      router.navigate(router.params.url, {
        reloadCurrent: true,
        pushState: false,
      });
    }
  ...
  /*
  pageBeforeRemove
  pageBeforeIn - before next->current or previous->current animation
  pageAfterIn - after next->current or previous->current animation
  pageBeforeOut - before current->next or current->previous
  pageAfterOut - after current->next or current->previous
  */
  ...
  // pageBeforeAnimationCallback(el, position, route) {
  //   if (!el) return;
  //   const router = this;
  //   const $el = $(el);
  ...
  //   router.emit('pageBeforeAnimation', page);
  //   $el.trigger('pageBeforeAnimation page:beforeanimation', page);
  // }
  // pageAfterAnimationCallback(el, position, route) {
  //   if (!el) return;
  //   const router = this;
  //   const $el = $(el);
  ...
  //   router.emit('pageAfterAnimation', page);
  //   $el.trigger('pageAFterAnimation page:afteranimation', page);
  // }
}
