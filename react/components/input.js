...
class F7Input extends React.Component {
  ...
  render() {
    ...
    let clearButtonEl;
    if (clearButton) {
      clearButtonEl = React.createElement('span', { className: 'input-clear-button' });
    }
    let infoEl;
    if (info || slotsInfo && slotsInfo.length) {
      infoEl = React.createElement('div', { className: 'item-input-info' }, info, this.slots['info']);
    }
    if (wrap) {
